...
""".strip()
    ...
    final_script = generate_script_response(prompt)
...
def generate_script_response(prompt: str, paragraph_number: int=1): 
...
3. you must only return the json-array of strings for terms. you must not return the script.
4. Allowed values for the category: backgrounds, fashion, nature, science, education, feelings, 
   health, people, religion, places, animals, industry, computer, food, sports, transportation, 
...
{{"terms": ["search term 1", "search term 2", "search term 3","search term 4","search term 5"], "category": "music"}}
...
""".strip()
    ...
    for i in range(_max_retries):
        try:
            ...
            search_terms = response_data.get("terms", [])
